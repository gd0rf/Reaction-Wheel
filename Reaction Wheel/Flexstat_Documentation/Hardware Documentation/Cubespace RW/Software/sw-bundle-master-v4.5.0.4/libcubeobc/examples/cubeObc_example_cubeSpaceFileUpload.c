...
int main(void)
{
	...
	TypesCommonFramework1_Identification identification;
	...
	if (identification.programType != TYPES_COMMON_FRAMEWORK_TYPES_1__PROGRAM_TYPE_BOOTLOADER)
	{
		printf("Reset to bootloader...\r\n");

		TypesCommonFramework1_Reset reset;

		reset.resetType = TYPES_COMMON_FRAMEWORK_1__SOFT;

		(void)tctlmCommonFramework1_setReset(&endpoint, &reset);
	...
	if (identification.nodeType != TYPES_COMMON_FRAMEWORK_TYPES_1__NODE_TYPE_LEGACY_CUBE_COMPUTER)
	{
		printf("Connected node is not CubeComputer!\r\n");
		return -1;
	}

	if (identification.programType != TYPES_COMMON_FRAMEWORK_TYPES_1__PROGRAM_TYPE_CONTROL)
	{
		if (identification.programType == TYPES_COMMON_FRAMEWORK_TYPES_1__PROGRAM_TYPE_BOOTLOADER)
		{
			...
			result =  cubeObc_common_pollForBootState(&endpoint, TYPES_COMMON_FRAMEWORK_1__APPLICATION_RUNNING, 500u, 10000u, FALSE);
